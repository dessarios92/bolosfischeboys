...
*/
  ...
  antes da execução e protege contra o SQL Injection,
    ...
    public static Connection conectar() throws SQLException {
        //Metodo responsavel por abrir a conexão com o banco de dados utilizando URL, usuário e senha fornecidos.
        return DriverManager.getConnection(bd_URL, bd_Usuario, bd_senha);
    }
    public static boolean testarValores(String valor, int tipoTeste) {
        // 1. Se o valor for nulo ou vazio, já retorna false aqui e encerra
        if (valor == null || valor.trim().isEmpty()) {
            return false;
        }

        // 2. Se NÃO for vazio, o fluxo continua para o switch de validação
        switch (tipoTeste) {
            case 1:
                // Tipo 1: Permite letras e espaços (A-Z, a-z e acentos básicos)
                return valor.matches("^[A-Za-zÀ-ÿ ]+$");
            case 2:
            case 3:
                // Tipo 2 e 3: Apenas números inteiros positivos
                return valor.matches("\\d+");
            case 4:
                // Tipo 4: Números decimais (Ex: 10; 10,50; 10.50)
                return valor.matches("\\d+([.,]\\d+)?");
            default:
                return false;
        }
    } // <- Fecha o método corretamente aqui
}
